  ...
  Future<void> getBorne() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        sendSecretket(key: generateSecretkey()); // genrate new secretkey;
...
//Emmettre un son
  ...
  Future<void> updateAllInfoForBorne() async {
    ...
    if (response.statusCode == 200) {
      ...
      developer.log("error de login avec le token");
    }
  }
  ...
  void startVisibleAnimation() {
    ...
    videoTimer.value =
      ...
      isCardVisible(false);
      developer.log(
          "suivant premiere fois ${currentArticleIndex.value.toString()} ${isCardVisible.value.toString()}");
  ...
  Future<void> getAllTicketForBorne() async {
    ...
    if (response.statusCode == 200) {
      ...
      developer.log("Erreur c'est produite pour les tickets");
    }
  }
  ...
  //Send secret key
  Future<void> sendSecretket({required String key}) async {
    tk.value = key;
    tk.refresh();
    await _borneService.sendSecretKey(key: key);
  }
...
//Iniatialisation du controlleur
  ...
  void onInit() async {
    ...
    ever(articles, (callback) {
      if (articles.isNotEmpty) {
        ...
        if (!isAnimating.value) {
          ...
          developer.log("l'article a ete modifier");
          update();
        }
      }
    });
    ...
    Timer.periodic(const Duration(hours: 48), (timer) {
      sendSecretket(key: generateSecretkey());
    });
  }
